...
vRP.prepare("characters/setMochila","UPDATE summerz_characters SET mochila = @mochila WHERE id = @user_id")
vRP.prepare("characters/getMochila","SELECT mochila FROM summerz_characters WHERE id = @id ")
vRP.prepare("instagram/getImage","SELECT id FROM summerz_characters WHERE image = @image")
