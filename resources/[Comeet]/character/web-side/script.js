$(document).ready(function(){
	...
	window.addEventListener('message',function(event){
		...
		if(event.data.openCharacter == true){
		...
		if(event.data.openCharacter == false){
	...
	$('input').change(function(){
		$.post('http://character/updateSkin',JSON.stringify({
			value: false,
			fathers: $('.fathers').val(),
			mothers: $('.mothers').val(),
			kinship: $('.kinship').val(),
	...
	document.onkeydown = function(data){
		if(data.which == 65){
			$.post('http://character/rotate',JSON.stringify("right"));
		}
		if(data.which == 68){
			$.post('http://character/rotate',JSON.stringify("left"));
		}
	}
});