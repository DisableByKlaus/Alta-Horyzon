...
function cRP.checkItem()
    ...
    if true then
        if Player.Functions.RemoveItem('resetperso',1) then
			TriggerClientEvent("Notify",source,"verde","Voce resetou o personagem com sucesso.",5000)
...
RegisterCommand("debug",function(source,args,rawCommand)
	if exports["chat"]:statusChat(source) then
		...
		if user_id then
			...
			TriggerClientEvent("character:apply",source,vRP.userData(user_id,"Barbershop"))
