...
local finalizoucorrida = false
local Position = 1

RegisterKeyMapping("+esconder","INICIAR CORRIDA","KEYBOARD","F7")
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADRACES
-----------------------------------------------------------------------------------------------------------------------------------------
						...
						if distance <= 10 then
							if Checkpoints >= #Races[Selected]["coords"] then
								if Laps >= Races[Selected]["laps"] then
									...
									local infos = vSERVER.GetPlayerName()
									local ids = vSERVER.GetPlayerID()
									SendNUIMessage({ info = infos, position = Position, id = ids,  time = ((ExplodeTimers - GetGameTimer()) / 1000)})
									SendNUIMessage({ show = false })
									SendNUIMessage({ score = true})
...
function makeBlips()
	...
	SetBlipColour(CheckBlip,1)
	SetBlipScale(CheckBlip,0.4)
	SetBlipAsShortRange(CheckBlip,false)
	SetBlipRoute(CheckBlip,false)
...
Citizen.CreateThread(function()
	while true do
		local sleep = 1
		if finalizoucorrida then
			sleep = 1
			drawTxt('~w~PRESSIONE ~r~F7 ~w~PARA ~g~FECHAR ~w~AS ESTATÍSTICA',4,0.295,0.905,0.4,255,255,255,200)
		end
		Citizen.Wait(sleep)
	end
end)

RegisterCommand("+esconder", function()
	SendNUIMessage({ score = false})
	finalizoucorrida = false
end)
