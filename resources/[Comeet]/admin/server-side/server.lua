...
RegisterCommand("god",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasGroup(user_id,"Suporte") or vRP.hasGroup(user_id,"Admin") then
			if args[1] then
				...
				if otherPlayer then
					...
					TriggerClientEvent('zSurvival:PlayerRevive',nuser_id) -- adicionar no teu
					vRPclient._removeObjects(source) -- adicionar no teu
				end
			else
				...
				vRPclient._removeObjects(source) -- adicionar no teu
				TriggerClientEvent('zSurvival:PlayerRevive',user_id) -- adicionar no teu
...
RegisterCommand("group",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasGroup(user_id,"Admin") or user_id == 1 or user_id == 2 and parseInt(args[1]) > 0 and args[2] then
...
RegisterCommand("fix",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasGroup(user_id,"Moderador") or vRP.hasGroup(user_id,"Admin") then
