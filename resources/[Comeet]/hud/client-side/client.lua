...
RegisterCommand("showmen", function(source --[[ this is the player ID (on the server): a number ]], args --[[ this is a table of the arguments provided ]], rawCommand --[[ this is what the user entered ]])
...
function updateDisplayHud()
	...
	if IsPedInAnyVehicle(ped) then
		...
		vidacar = GetVehicleEngineHealth(PedCar)
		local showBelt = true
		...
		SendNUIMessage({ vehicle = true, talking = talking, health = health, armour = armour, thirst = clientThirst, damageVehicle = vidacar, hunger = clientHunger, stress = 0, street = streetName, direction = flexDirection, radio = radioDisplay, voice = voice, oxigen = clientOxigen, suit = divingMask, fuel = fuel, speed = speed, showbelt = showBelt, seatbelt = beltLock, hardness = (hardness[plate] or 0) })
	else
		if IsMinimapRendering() then
			DisplayRadar(false)
		end

		SendNUIMessage({ vehicle = false, talking = talking, health = health, armour = armour, thirst = clientThirst, hunger = clientHunger, stress = 0, street = streetName, direction = flexDirection, radio = radioDisplay, voice = voice, oxigen = clientOxigen, suit = divingMask })
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD
